...
- An Open-Source distributed data stream platform used for high performance in data pipeline, analytics stream, data integration and mission critical applications
...
- Good use for 2 contexts: Kafka is not a solution that has to be implemented only in companies that have high volumetry of data. It can be implemented in companies that have a lot of services communicating with each other.
- It works in cluster
...
## Questions:
  ...
  - How we scale?
  - How can we have resilience and high availability? (systems cannot lose messages)
 
## Advantages:
 - High throughput (capacity of receive and process requests): it can handle million requests per second
 - Latency is extremely low (2ms)
 - Scalable
 - Storage: It has a database area that keeps all the data. It is optimized and secure
 - High Availability
...
## How it works and important things:
 ...
 - Kafka does not distribute messages. The Consumer is responsible for get it there.
 - Minimal recommendation for production: 3 brokers (3 nodes, 3 machines)
 - The brokers communicate with each other
 - Zookeper: it is a software that helps the brokers to communicate with each other. However, Kafka is acquiring its own independency and probably it will not be used in the future

## Differences between Kafka and RabbitMQ
 - Kafka is different from RabbitMQ. RabbitMQ is Queue. Kafka is Topic
 - A Topic can be read by a lot of systems (a lot of systems can read the same topic). A Queue delivers the message to a unique consumer (once the message is delivered, the message is gone of the queue).
...
## Topics
  ...
  ```
  1) Header: Metadata, it's not mandatory
  2) Key: Message Type
  3) Value: Content (json)
  4) Timestamp
  ```
## Partitions
  - A way to scale the topic. The message is processed faster because of the number of brokers. Each Topic can have one or more partitions to ensure the distribution and resiliency of its data. The information becomes more distributed. It is used to Kafka scale the topic
  - Topic -> Partitions (each one  may be in a different broker (node))
  - Collateral effect: how do we assure the order of the messages? Two messages that have an order have to go to the same partition! To do it, we have to define the "key" of the message. Ex: Key "MoneyMovement", has a "transfer" and "cash back". Transfer has to be first than the cash back. So, to guarantee the order, we use the same key for the message. If the key is not informed, the messages will be sent to the partitions dynamically.
  - Replication Factor: Ex: If selected 2 Replicator Factors, It will have 2 copies of each partition summing all brokers. It is used to have a backup. Helo to guarantee resiliency. Usually used 2 or 3 (3 is usually for very critical systems). Ex of 2 Replication Factors:
  ```
  ...
  - Partition Follower: When the partition leader drops, the consumer will get the information from the follower (we do not set it, kafka sets with its algorithm)

## Delivery Guarantee
  - Ack 0 (None): When we are OK that some messages cannot be delivered, we use FF (Fire and Forget) (This way is fast because we do not need a confirmation that the message was delivered into the broker)
  - Ack 1 (Leader): The leader saves the message and sends an answer to the producer that the message was delivered (it is slower). However, if the Leader Broker lost its message (some bug), and did not sent it to the follower brokers, you receive a confirmation and you are going to think that the message was not lost
...
## Consumer Groups
  ...
  - Used in more than one consumer (the same software), however, working on different machines or in different processes
  ...
  - Inside the same group, there will be one consumer per partition. If there are more consumers than partitions, the last consumers will be idle.

## Commands
  - run "docker-compose up" and go inside the bash of the kafka "docker exec -it kafka_kafka_q bash"
...
## Kafka Connect
  ...
  - It´s very simple. It does not need to develop anything.
  - Connectors:
  ...
  - Distributed Workers: (more than one node)
    * The workers help each other to execute the tasks. They slice the tasks
    * The workers have to be in the same group (the same group id). It is very similar with the consumer group
    * Structure: 
  ...
  - Dead Letter Queue (DLQ):
    * Sometimes we get data but it is not in the exact format that we want causing some error (or other error). DLQ is the local that we send the message to analyze later.
    * property: "errors.tolerance". This configuration is only applied for connectors of type "Sink"
      - Types:
        * NONE: the task fails instantly. The task stops.
        * ALL: errors are ignored and the process keeps going
        * errors.deadletterqueue.topic.name = topic_name: it has to be "ALL". Everytime that has an error, this error goes to a topic. Inside the topic, the message will come with some headers. And inside these headers we can see the log and the explanation for the error.
...
## Other informations:
- KsqlDB: It's like SQL in Kafka. We can get data more personalized, create relationships between tables. Beyond this, we can get some real time information that things that are happening in the system that is going to the kafka
